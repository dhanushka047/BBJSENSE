...
void loop() {
  ...
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (currentMs - lastTelemetryMs >= TELEMETRY_INTERVAL) {
      ...
      // Simulated analog and digital readings for offline logs
      float scaledVal[4];
      static float mockOfflineAngle = 0;
      mockOfflineAngle += 0.1f;
      if (mockOfflineAngle > 360.0f) mockOfflineAngle = 0;
      
      scaledVal[0] = analogConfigs[0].minValue + (analogConfigs[0].maxValue - analogConfigs[0].minValue) * (0.5f + 0.5f * sin(mockOfflineAngle));
      scaledVal[1] = analogConfigs[1].minValue + (analogConfigs[1].maxValue - analogConfigs[1].minValue) * (0.5f + 0.5f * cos(mockOfflineAngle * 0.5f));
      scaledVal[2] = analogConfigs[2].minValue + (analogConfigs[2].maxValue - analogConfigs[2].minValue) * ((float)random(200, 800) / 1000.0f);
      scaledVal[3] = analogConfigs[3].minValue + (analogConfigs[3].maxValue - analogConfigs[3].minValue) * ((float)(millis() % 10000) / 10000.0f);
      ...
      offlineDoc["digital_in1"] = (millis() % 6000 < 3000);
      offlineDoc["digital_in2"] = (millis() % 10000 < 5000);
      offlineDoc["digital_in3"] = (random(0, 2) == 1);
      offlineDoc["digital_in4"] = false;
...
void checkI2CBus() {
  ...
  if (Wire.endTransmission() != 0) {
    Serial.println("[MOCK] ADS1115 ADC check bypassed (Simulated).");
  } else {
    if (adc.begin(Wire)) {
  ...
  if (Wire.endTransmission() != 0) {
    Serial.println("[MOCK] PCF8563 RTC check bypassed (Simulated).");
  } else {
    rtc.begin(Wire, SDA_PIN, SCL_PIN);
  }

  i2cError = false; // Always report normal operational status in mock mode
  Serial.println("[OK] I2C Bus responding normally (MOCK MODE).");
}
...
void processTelemetry() {
  ...
  float scaledVal[4];
  static float mockAngle = 0;
  mockAngle += 0.1f;
  if (mockAngle > 360.0f) mockAngle = 0;
  
  // Simulated analog inputs scaled relative to their current configuration ranges
  scaledVal[0] = analogConfigs[0].minValue + (analogConfigs[0].maxValue - analogConfigs[0].minValue) * (0.5f + 0.5f * sin(mockAngle));
  scaledVal[1] = analogConfigs[1].minValue + (analogConfigs[1].maxValue - analogConfigs[1].minValue) * (0.5f + 0.5f * cos(mockAngle * 0.5f));
  scaledVal[2] = analogConfigs[2].minValue + (analogConfigs[2].maxValue - analogConfigs[2].minValue) * ((float)random(200, 800) / 1000.0f);
  scaledVal[3] = analogConfigs[3].minValue + (analogConfigs[3].maxValue - analogConfigs[3].minValue) * ((float)(millis() % 10000) / 10000.0f);

  // Simulated digital inputs (periodic toggling)
  bool dIn1 = (millis() % 6000 < 3000);
  bool dIn2 = (millis() % 10000 < 5000);
  bool dIn3 = (random(0, 2) == 1);
  bool dIn4 = false;
...
bool readModbusRegister(uint8_t slaveId, uint16_t address, uint8_t fc, uint8_t *respData, int byteCount) {
  // Mock Modbus read: fill response buffer with simulated values
  for (int i = 0; i < byteCount; i++) {
    respData[i] = (uint8_t)random(0, 256);
  }
  // If reading float or 32-bit types (4 bytes), generate a realistic simulated value
  if (byteCount == 4) {
    float val = 25.5f + (float)random(0, 100) / 10.0f; // E.g., simulated temperature around 25.5-35.5 C
    ...
    // 16-bit register: generate random integer between 100 and 500
    uint16_t val = (uint16_t)random(100, 501);
    respData[0] = (val >> 8) & 0xFF;
    respData[1] = val & 0xFF;
  }
  delay(50); // Simulate network/Modbus communication latency
  return true;
}
